...
export async function handleDeleteCategory(env, user, categoryId, corsHeaders) {
  if (!user || user.role !== 'super_admin') {
    return new Response(JSON.stringify({ error: 'Unauthorized' }), {
      status: 403,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
  ...
  try {
    ...
    // Check if any sites use this category
    const sitesCount = await env.DB.prepare(
    ...
    if (sitesCount && sitesCount.count > 0) {
      return new Response(JSON.stringify({ 
        ...
        message: `${sitesCount.count} site(s) are using this category. Please reassign or delete those sites first.`
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
